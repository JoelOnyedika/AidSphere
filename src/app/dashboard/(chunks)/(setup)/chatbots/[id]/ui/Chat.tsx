...
const Chat = () => {
  ...
  const chatWelcomeMessage = useRecoilValue(chatWelcomeMessageTextStateSelector)
  ...
  return  (
    <div  className={`${chatBackgroundTheme === "light" ? "bg-white" : "bg-slate-800"} rounded-2xl overflow-hidden w-[470px] h-[700px] flex flex-col`}>
      <div style={{background: `rgb(${chatBrandColor})`}} className="rounded-t-2xl flex flex-col justify-center px-6 py-10 items-center">
      ...
      <div className="m-4 rounded-lg border border-solid border-gray-700 flex">
        <Input
          className={`${chatBackgroundTheme === "light" ? "text-slate-800" : "text-white"} rounded-lg border border-none focus: border-gray-700`}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant={'ghost'}>
                <Send className={`${chatBackgroundTheme === "light" ? "text-black" : "text-white"} scale-75 p-0`} />
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>Send</p>
